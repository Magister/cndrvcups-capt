...
2.Canon CAPT Printer Driver for Linuxの配布ファイル構成 ------------------------
  ...
  - cndrvcups-common-3.20-x.i386.rpm（32-bit用）
  - cndrvcups-common-3.20-x.x86_64.rpm（64-bit用）
  - cndrvcups-common_3.20-x_i386.deb（Debian 32-bit用）
  - cndrvcups-common_3.20-x_amd64.deb（Debian 64-bit用）
    Canon CAPT Printer Driver for Linuxで用いる、CUPSドライバ共通モジュールの
    インストールパッケージです。

  - cndrvcups-capt-2.70-x.i386.rpm（32-bit用）
  - cndrvcups-capt-2.70-x.x86_64.rpm（64-bit用）
  - cndrvcups-capt_2.70-x_i386.deb（Debian 32-bit用）
  - cndrvcups-capt_2.70-x_amd64.deb（Debian 64-bit用）
...
- cndrvcups-commonパッケージのバージョン3.20をインストールする場合、
  cndrvcups-captパッケージのバージョン2.70をインストールしてお使いください。
...
- プリンタドライバを1.90以前のバージョンから、バージョン2.70にアップデート
  インストールする場合、アップデートインストール前に登録されているプリンタは、
  再登録する必要があります。

- debパッケージのプリンタドライバを1.90以前のバージョンから、バージョン2.70に
